...
int32_t test_hmac_sha1_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl, uint32_t nvals, uint32_t nthreads, e_mt_gen_alg mt_alg, e_sharing sharing) {
	...
	s_hash_out = BuildHMACSHA1Circuit(s_msgSi, s_msgSo, s_msgC, msgSi.GetArr(), msgSo.GetArr(), msgC.GetArr(), plain_out, nvals, (BooleanCircuit*) circ);
...
share* BuildHMACSHA1Circuit(share* s_msgSi, share* s_msgSo, share* s_msgC, uint8_t* msgSi, uint8_t* msgSo, uint8_t* msgC, uint8_t* plain_out, uint32_t nvals, BooleanCircuit* circ) {
